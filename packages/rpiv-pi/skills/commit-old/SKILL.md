---
name: commit-old
description: Pre-migration baseline of the commit skill, kept temporarily for side-by-side comparison with `/skill:commit`. No shell substitution — the LLM issues `git status --short` and `git diff` itself. Invoke explicitly as `/skill:commit-old`. Remove once comparison is complete.
